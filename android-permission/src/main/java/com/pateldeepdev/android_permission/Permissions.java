package com.pateldeepdev.android_permission;
...
public class Permissions {
    ...
    /**
     * Check a permission and return boolean value
     *
     * @param context    the android context.
     * @param permissions the permission to be requested.
     */
    ...
    /**
     * Check a single permission permission and return boolean value
     *
     * @param context    the android context.
    ...
    /**
     * Check/Request a multiple permission permission and call the callback methods of permission handler accordingly.
     *
     * @param context    the android context.
     * @param permission the permission to be requested.
     ...
     */
    public static void request(Context context, String permission, String rationale,
                             PermissionHandler handler) {
        request(context, new String[]{permission}, rationale, null, handler);
    }
     ...
     *                    actions such as permission granted, permission denied, etc.
     */
    public static void request(Context context, String permission, int rationaleId,
                             PermissionHandler handler) {
     ...
     *                    actions such as permission granted, permission denied, etc.
     */
    public static void request(final Context context, String[] permissions, String rationale,
        ...
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.M) {
            ...
            if (allPermissionProvided) {
                handler.onGranted();
                log("Permission(s) " + (PermissionUtil.permissionHandler == null ?
                        "already granted." : "just granted from settings."));
                PermissionUtil.permissionHandler = null;

            } else {
                PermissionUtil.permissionHandler = handler;
                ArrayList<String> permissionsList = new ArrayList<>(permissionsSet);

                Intent intent = new Intent(context, PermissionUtil.class)
                        .putExtra(PermissionUtil.EXTRA_PERMISSIONS, permissionsList)
                        .putExtra(PermissionUtil.EXTRA_RATIONALE, rationale)
                        .putExtra(PermissionUtil.EXTRA_OPTIONS, options);
     ...
     *                    actions such as permission granted, permission denied, etc.
     */
    public static void request(final Context context, String[] permissions, int rationaleId,
                             Options options, final PermissionHandler handler) {
        String rationale = null;
        try {
            rationale = context.getString(rationaleId);
        ...
        request(context, permissions, rationale, options, handler);
    }
